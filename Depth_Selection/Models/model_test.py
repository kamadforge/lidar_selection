...
class Model(nn.Module):
    def __init__(self, args):
        ...
        self.depthnet = Net(args, out_channels=2, drop=dropout)
        local_channels_in = 2 
        self.convbnrelu1 = nn.Sequential(conv(local_channels_in, 32, 3, 1, 1, 1),
                                        nn.ReLU(inplace=True))
        # self.convbnrelu2 = nn.Sequential(conv(32, 32, 3, 1, 1, 1),
                                        # nn.ReLU(inplace=True))
...
class hourglass_1(nn.Module):
    ...
    def forward(self, x, em1, em2):
        ...
        x_prime = self.maxpool(x)
        x_prime = self.conv3(x_prime)
...
class hourglass_2(nn.Module):
    def __init__(self, channels_in):
        ...
        self.maxpool=nn.MaxPool2d(2, 2)
        self.conv1 = nn.Sequential(convbn(channels_in, channels_in*2, kernel_size=3, stride=1, pad=1, dilation=1),
                                   nn.ReLU(inplace=True))
