...
class DepthCompletionNet(nn.Module):
    ...
    def forward(self, x):
        ...
        print("convt5 shape", convt5.shape)
...
class DepthCompletionNetInMain(nn.Module):
    def __init__(self, args):
        assert (
            args.layers in [18, 34, 50, 101, 152]
        ), 'Only layers 18, 34, 50, 101, and 152 are defined, but got {}'.format(
            layers)
        super(DepthCompletionNetInMain, self).__init__()
        self.modality = args.input
...
class DepthCompletionNetIn(nn.Module):
    def __init__(self, args):
        assert (
            args.layers in [18, 34, 50, 101, 152]
        ), 'Only layers 18, 34, 50, 101, and 152 are defined, but got {}'.format(
            layers)
        super(DepthCompletionNetIn, self).__init__()
        self.modality = args.input
        ...
        if args.layers <= 34:
            ...
            num_channels = 2048
        self.conv6 = conv_bn_relu(num_channels,256,kernel_size=3,stride=2,padding=1)
        self.conv7 = conv_bn_relu(256,256,kernel_size=3,stride=2,padding=1)
        self.conv8 = conv_bn_relu(256,256,kernel_size=3,stride=2,padding=1)


        self.fc = nn.Linear(in_features=256*6*19, out_features=65)

        # switch mask
        self.parameter_mask = torch.Tensor(np.load("features/kitti_pixels_to_lines_masks.npy", allow_pickle=True)).to(
            device)
    ...
    def forward(self, x):
        ...
        conv7 = self.conv7(conv6)  # batchsize * ? *
        conv8 = self.conv8(conv7)  # batchsize * ? *
...
class DepthCompletionNetQSquare(nn.Module):
    ...
    def forward(self, x):
        ...
        # first layer
        if 'd' in self.modality:
            print(f"depth (sparse) input to the network: {len(torch.where(x['d']>0)[0])}")
            conv1_d = self.conv1_d(output)
        if 'rgb' in self.modality:
            conv1_img = self.conv1_img(x['rgb'])
        elif 'g' in self.modality:
