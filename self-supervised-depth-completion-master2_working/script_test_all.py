...
def test_switch(checkpoint_path, rank_file):
...
def test_features_in_checkpoint(checkpoint_path):
    ...
    for tm in test_modes:
        for fm in feat_modes:
            for fn in feat_nums:
                print("\n\n{tm}  {tf}  {fn}\n\n")
                subprocess.run(["/home/kadamczewski/miniconda3/bin/python", "/home/kadamczewski/Dropbox_from/Current_research/depth_completion/self-supervised-depth-completion-master2_working/main_orig_jul26.py", "--data-folder", "/is/cluster/scratch/kamil/kitti", "--i", "gd", "--type_feature", "sq", "--train-mode", "dense", "--workers", "0", "-e" ,checkpoint_path, "--seed", "121", "--test_mode", tm, "--feature_mode", fm,  "--feature_num", fn])